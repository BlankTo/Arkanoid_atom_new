...
def convert_to_phenomenon(event_or_unexplained):
    ...
    if isinstance(event_or_unexplained, UnexplainedSpecificChange):
        ...
        if previous == 0 or previous is None:
            ...
            a = 0
            b = final
            phenoms = [NumericalUnexplainedPhenomenon({'a': a, 'b': b, 'prop_name': event_or_unexplained.prop_name})]
            #print(f'a: {a}, b: {b}')
        elif final == 0:
            #print('final == 0')
            a = 0
            ...
            #a2 = 0
            #b2 = final
            phenoms = [NumericalUnexplainedPhenomenon({'a': a1, 'b': b1, 'prop_name': event_or_unexplained.prop_name}),
                       #NumericalUnexplainedPhenomenon({'a': a2, 'b': b2, 'prop_name': event_or_unexplained.prop_name}),
                       ]
            #print(f'a1: {a1}, b1: {b1}')
            #print(f'a2: {a2}, b2: {b2}')
        return phenoms, 1
...
def new_infer_rules(obj, frame_id, debug= False):
    ...
    for (effect_hash, fid), conflicting_rules_tuples in effect_frame.items():
        ...
        if len(conflicting_rules_tuples) > 1:
            ...
            rule_hash = best_rule.my_hash()
            if rule_hash not in new_new_seen_rules:
                new_new_potential_rules.append(best_rule)
                new_new_seen_rules.append(rule_hash)
                obj_explained_score -= best_times
                obj_rules_score.append((best_rule, best_times))
        else:
            rule_hash = conflicting_rules_tuples[0][0].my_hash()
            if rule_hash not in new_new_seen_rules:
                new_new_potential_rules.append(conflicting_rules_tuples[0][0])
                new_new_seen_rules.append(rule_hash)
                obj_explained_score -= conflicting_rules_tuples[0][1]
                obj_rules_score.append((conflicting_rules_tuples[0][0], conflicting_rules_tuples[0][1]))
